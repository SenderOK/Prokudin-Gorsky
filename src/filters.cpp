...
PrecisePixel CustomFilter::operator()(const PreciseImage &m) const
{
    double ker, r, g, b, ans_r = 0, ans_g = 0, ans_b = 0;
    ...
    return make_tuple(ans_r, ans_g, ans_b);
}
...
Pixel ClipPixelFilter::operator()(const PreciseImage &m) const
{
    double r, g, b;
    uint r_i, g_i, b_i;
    tie(r, g, b) = m(0, 0);
    r_i = uint(std::min(std::max(r, 0.0), 255.0));
    g_i = uint(std::min(std::max(g, 0.0), 255.0));
    b_i = uint(std::min(std::max(b, 0.0), 255.0));
    return make_tuple(r_i, g_i, b_i);
}
