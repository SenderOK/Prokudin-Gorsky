...
public:
    ...
    friend Matrix<typename std::result_of<BinaryMatrixOperator(ValueT, ValueT)>::type>
    binary_map(const BinaryMatrixOperator& op, const Matrix<ValueT>& m1, const Matrix<ValueT>& m2)
    {
        // Let's typedef return type of function for ease of usage
        typedef typename std::result_of<BinaryMatrixOperator(ValueT, ValueT)>::type ReturnT;
   
        if(m1.n_rows != m2.n_rows || m1.n_cols != m2.n_cols)
            throw std::string("binary_map: matrices must be of the same size");
       
        Matrix<ReturnT> result(m1.n_rows, m1.n_cols);
    ...
    template<typename TernaryMatrixOperator>
    friend Matrix<typename std::result_of<TernaryMatrixOperator(ValueT, ValueT, ValueT)>::type>
    ternary_map(const TernaryMatrixOperator& op, const Matrix<ValueT>& m1, const Matrix<ValueT>& m2, const Matrix<ValueT>& m3)
    {
        // Let's typedef return type of function for ease of usage
        typedef typename std::result_of<TernaryMatrixOperator(ValueT, ValueT, ValueT)>::type ReturnT;
   
        if(m1.n_rows != m2.n_rows || m1.n_rows != m3.n_rows || m1.n_cols != m2.n_cols || m1.n_cols != m3.n_cols)
            throw std::string("ternary_map: matrices must be of the same size");
       
        Matrix<ReturnT> result(m1.n_rows, m1.n_cols);
   
        for (uint i = 0; i < m1.n_rows; ++i)
            for (uint j = 0; j < m1.n_cols; ++j) 
                result(i, j) = op(m1(i, j), m2(i, j), m3(i, j));
    ...
    Matrix<ValueT> MirrorExpand(uint n_new_rows, uint n_new_cols) const;
    
    Matrix<ValueT> rotate_clockwise(int n) const;

private:
    Matrix<ValueT> rotate1() const;
